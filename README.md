...
2. 切到 **「快捷操作」** Tab，用上方按钮搭流程：
   ...
   - **＋点击** 录购买位置（每轮连点可填 3~5）；要分支再加 **＋否则 / ＋结束**，缩进即 if/else。
   - 触发方式默认「每轮判断」；等价格弹窗要买一次别重复就改「仅响应变化」。
   - 勾停止条件：命中（买 N 次）/ 轮数 / 分钟，可多选。
...
**已实现**：「快捷操作」页 = 可视化流程构建器（点击 / 检测(if) / 否则 / 结束，if/else 缩进、整轮循环）；检测支持同区域多条 且/或 子条件 + 「测试识别」+ 触发方式（每轮判断 / 仅响应变化）；停止条件多选（点击 / 命中 / 轮数 / 分钟 / 检测"满足即停"）；流程保存/加载；窗口可自由拉宽；HumanClicker 全参数配置化（高级设置）；原版步骤编辑器原功能；`.NET 8` 编译通过（本机 8.0.424）。
